...
export const LiveRadarMap: React.FC<LiveRadarMapProps> = ({
  ...
  const selectedSession = useMemo(() => {
    ...
    return (
      filteredSessions.find((s) => s.status === "emergency") ||
      filteredSessions[0] ||
      null
    );
  ...
  const matchingReport = useMemo(() => {
    ...
    return (
      reports.find(
        (r) =>
          (r.reportedBy &&
            (r.reportedBy === selectedSession.userId ||
              r.reportedBy === selectedSession.id)) ||
          (r.reporterName &&
            selectedSession.userName &&
            r.reporterName.toLowerCase() ===
              selectedSession.userName.toLowerCase()) ||
          (r.source === "safety_mode_emergency" &&
            selectedSession.userName &&
            r.title.includes(selectedSession.userName)),
      ) || null
    );
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const activeHandlesRef = useRef<
    Map<
      string,
      {
        marker: any;
        overlay: any;
        update: (s: SafetySession, sel: boolean) => void;
        destroy: () => void;
      }
    >
  >(new Map());
  ...
  const displayedQueue = useMemo(
    () =>
      filteredSessions.filter((s) => {
        if (queueFilter === "emergency" && s.status !== "emergency")
          return false;
        if (queueFilter === "distress" && s.status !== "distress_pending")
          return false;
  ...
  const handleCommunitySelect = useCallback(
    (val: string) => {
      ...
      if (
        placeMatch &&
        placeMatch.lat !== undefined &&
        placeMatch.lng !== undefined
      ) {
        const reg = registerDynamicCommunity(
          jurisdictionSettings.countryCode,
          jurisdictionSettings.stateCode,
  ...
  const smoothFlyTo = useCallback(
    (targetCenter: { lat: number; lng: number }, targetZoom: number) => {
      if (engine === "osm") {
      ...
      const frame = (now: number) => {
        ...
        if (startZoom !== targetZoom) {
          ...
          if (dist > 1.2 && progress > 0.15 && progress < 0.85) {
            const altitudeLift =
              Math.sin(progress * Math.PI) * Math.min(2.5, dist * 0.35);
  ...
  const handleSelectSession = useCallback(
    (sess: SafetySession) => {
      ...
      smoothFlyTo(
        { lat: sess.currentLocation.lat, lng: sess.currentLocation.lng },
        15.5,
      );
    },
  ...
  useEffect(() => {
    ...
    const targetZoom = Math.round(
      activePerimeter.zoom || (activePerimeter.level === "global" ? 3 : 6),
    );
  ...
  useEffect(() => {
    ...
    class FloatingLocationChip extends googleObj.maps.OverlayView {
      ...
      onAdd() {
        ...
        chip.style.borderColor = this.isSelected
          ? "var(--primary)"
          : isEmergency
            ? "var(--alert-red)"
            : "var(--border-hairline)";
        chip.style.boxShadow = this.isSelected
      ...
      update(
        ...
        if (this.div) {
          ...
          const ring = this.div.querySelector(
            ".radar-ring",
          ) as HTMLElement | null;
          if (ring) {
            ring.className = isEmergency
          ...
          const chip = this.div.querySelector(
            ".gmap-chip-tag",
          ) as HTMLElement | null;
          if (chip) {
            chip.style.borderColor = isSelected
              ? "var(--primary)"
              : isEmergency
                ? "var(--alert-red)"
                : "var(--border-hairline)";
      ...
      draw() {
        ...
        try {
          const pos = overlayProjection.fromLatLngToDivPixel(
            new (googleObj as any).maps.LatLng(
              this.session.currentLocation.lat,
              this.session.currentLocation.lng,
            ),
          );
          if (pos) {
  ...
  useEffect(() => {
    ...
    if (engine === "osm") {
      ...
      filteredSessions.forEach((sess) => {
        ...
        if (existing) {
          existing.setLatLng([
            sess.currentLocation.lat,
            sess.currentLocation.lng,
          ]);
          existing.setIcon(icon);
          existing.setZIndexOffset(isEmergency ? 1000 : isDistress ? 500 : 100);
        } else {
          const marker = L.marker(
            [sess.currentLocation.lat, sess.currentLocation.lng],
            {
              icon,
      ...
      filteredSessions.forEach((sess) => {
        ...
        if (existing) {
          existing.marker.setPosition({
            lat: sess.currentLocation.lat,
            lng: sess.currentLocation.lng,
          });
          ...
          handles.set(sess.id, {
            marker,
            overlay: chip,
            update: (s: SafetySession, sel: boolean) =>
              chip.update(s, sel, handleSelectSession),
            destroy: () => {
              marker.setMap(null);
              chip.setMap(null);
            },
  ...
  useEffect(() => {
    ...
    if (engine === "osm") {
      ...
      if (
        jurisdictionSettings.mode === "global" ||
        activePerimeter.level === "global"
      )
        return;
      ...
      const renderPaths = (paths: { lat: number; lng: number }[][]) => {
        ...
        paths.forEach((ring) => {
          if (ring.length >= 3) {
            const latLngs: L.LatLngExpression[] = ring.map((p) => [
              p.lat,
              p.lng,
            ]);
      ...
      async function loadOsmBoundary() {
        ...
        const level =
          activePerimeter.level === "country"
            ? "country"
            : activePerimeter.level === "state"
              ? "state"
              : "community";
        const geoJson = await fetchRealBoundaryGeoJson(
          query,
          activePerimeter.center,
          level,
        );
  ...
  const handleZoom = useCallback(
    (direction: "in" | "out") => {
      if (engine === "osm" && leafletMapRef.current) {
        ...
        const center =
          googleMapInstanceRef.current.getCenter()?.toJSON() ??
          (activePerimeter.center || { lat: 20, lng: 10 });
  ...
  const handleRecenter = useCallback(() => {
    if (selectedSession) {
      smoothFlyTo(
        {
          lat: selectedSession.currentLocation.lat,
          lng: selectedSession.currentLocation.lng,
        },
        15.5,
      );
  ...
  return (
    ...
    >
      ...
      <div
        className="map-side-panel radar-left-panel"
        style={{
            ...
            <div style={{ display: "flex", gap: "0.25rem" }}>
              <button
                ...
                style={{
                  ...
                  border: "none",
                  backgroundColor:
                    queueFilter === "all" ? "var(--text-main)" : "#FFFFFF",
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  backgroundColor:
                    queueFilter === "active" ? "var(--primary)" : "#FFFFFF",
                  color:
                    queueFilter === "active" ? "#FFFFFF" : "var(--text-sub)",
                  cursor: "pointer",
                  boxShadow: "var(--shadow-sm)",
                }}
              >
            ...
            {displayedQueue.length === 0 ? (
              ...
              >
                {globalSearch
                  ...
                  : "No active users in current view."}
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ margin: "0.35rem 0", paddingLeft: "1.2rem" }}
                    >
                      <div
                        className="route-timeline-line"
                        style={{ left: "5px", top: "7px", bottom: "7px" }}
                      />
                      ...
                      >
                        <div
                          className="route-node-icon-start"
                          style={{
                            left: "-1.2rem",
                            width: "13px",
                            height: "13px",
                        ...
                        >
                          {new Date(sess.lastPingAt).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                        </div>
                    ...
                    >
                      ...
                      >
                        <a
                          href={
                            sess.userPhone ? `tel:${sess.userPhone}` : undefined
                          }
                          onClick={(e) => {
                            e.stopPropagation();
      ...
      >
        ...
        <div
          className="radar-jurisdiction-bar"
          style={{
            position: "absolute",
            top: "1rem",
            left: "1rem",
      ...
      <div
        className="map-side-panel radar-right-panel"
        style={{
          ...
          transform:
            isInspectorCollapsed || !selectedSession
              ...
              : "translateX(0)",
          pointerEvents:
            isInspectorCollapsed || !selectedSession ? "none" : "auto",
          marginLeft:
            isInspectorCollapsed || !selectedSession ? "-0.75rem" : "0",
          display: "flex",
          flexDirection: "column",
        }}
      >
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <Battery
                    size={12}
                    color={
                      selectedSession.batteryLevel < 25
                        ? "var(--alert-red)"
              ...
              >
                ...
                >
                  {new Date(selectedSession.lastPingAt).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </div>
            ...
            >
              ...
              >
                {selectedSession.currentLocation.lat.toFixed(5)},{" "}
                {selectedSession.currentLocation.lng.toFixed(5)}
              </div>
            </div>
            ...
            >
              ...
              <a
                ...
                style={{
                  ...
                  gap: "0.4rem",
                  textDecoration: "none",
                  color: selectedSession.userPhone
                    ? "var(--text-main)"
                    : "var(--text-muted)",
                ...
                title={
                  selectedSession.userPhone
                    ? `Dial ${selectedSession.userPhone}`
