...
export const ReportDetailPage: React.FC = () => {
  ...
  if (!report) {
    return (
      <div style={{ padding: '4rem 2rem', textAlign: 'center', backgroundColor: '#F4F7F5', minHeight: '100%' }}>
        <div style={{
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.35rem', marginBottom: '0.5rem' }}>
            Incident Report Not Located
          </h2>
          <p style={{ color: 'var(--text-muted)', marginBottom: '1.75rem', fontSize: '0.85rem' }}>
            The requested incident report #{id} could not be found in active memory.
          </p>
  ...
  const handleSendComment = (e: React.FormEvent) => {
    ...
    addComment(report.id, commentInput.trim());
    setCommentInput('');
  };
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem', flexWrap: 'wrap' }}>
              <span className={`badge ${
                report.urgency === 'critical' ? 'badge-emergency' : report.urgency === 'high' ? 'badge-distress' : 'badge-neutral'
              }`}>
            ...
            {/* Metadata Bar */}
            <div style={{ display: 'flex', gap: '1.5rem', fontSize: '0.8125rem', color: 'var(--text-sub)', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                {report.isAnonymous ? <EyeOff size={14} /> : <User size={14} color="var(--primary)" />}
                <span style={{ fontWeight: 600 }}>{report.isAnonymous ? 'Anonymous' : report.reporterName || 'User'}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--text-main)', fontFamily: 'var(--font-display)' }}>
              STATUS:
            </span>
            <Select
              showSearch
              ...
              value={report.status}
              onChange={(val) => updateReportStatus(report.id, val as IncidentStatus)}
              options={[
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: '0.8125rem', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '2px' }}>
                  <MapPin size={15} color="var(--primary)" />
                  <span>Location Details</span>
                </div>
                <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>
                  Latitude: {report.location.lat.toFixed(5)} • Longitude: {report.location.lng.toFixed(5)}
                </div>
              </div>
              ...
              <a
                ...
                className="btn-oneforma-ghost"
                style={{ padding: '8px 14px', fontSize: '0.75rem', borderRadius: 'var(--radius-pill)' }}
              >
                  ...
                  report.staffComments.map((comm) => (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.35rem', fontSize: '0.75rem' }}>
                        <span style={{ fontWeight: 800, color: 'var(--accent-green-dark)' }}>
                          {comm.staffName} <span style={{ color: 'var(--text-muted)', fontWeight: 500 }}>({comm.staffRole})</span>
                        </span>
                      ...
                      <p style={{ fontSize: '0.8125rem', color: 'var(--text-sub)', margin: 0, lineHeight: 1.45 }}>
                        {comm.comment}
                      </p>
                    </div>
                  ))
